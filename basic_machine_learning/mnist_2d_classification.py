...
# Note:
#   - BN eliminates the need for Dropout in some cases cause BN provides similar regularization benefits
#     as Dropout intuitively
#   - Architectures like ResNet, DenseNet, etc. not using Dropout
model.add(layers.Conv2D(16, kernel_size=5, activation='relu', input_shape=(28, 28, 1)))
...
x_train = x_train.reshape((-1, 28, 28, 1)).astype('float32') / 255  # Conv2D layer requires 4D input
...
model.compile(optimizer=keras.optimizers.SGD(), loss=loss, metrics=['sparse_categorical_accuracy'])

filepath = "mnist-2d.hdf5"
callbacks = [
